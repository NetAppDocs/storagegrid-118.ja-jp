...
リカバリパッケージ `.zip` ファイル。
 a| 
実行する必要があります link:downloading-recovery-package.html["最新のリカバリパッケージをダウンロードします"] `.zip` ファイル。 (`sgws-recovery-package-_id-revision_.zip`）。リカバリパッケージファイルは、障害発生時のシステムのリストアに使用できます。
